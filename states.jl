function POMDPs.stateindex(pomdp::DronePOMDP{K}, s::RSState{K}) where K
...
function state_from_index(pomdp::DronePOMDP{K}, si::Int) where K
    if si == length(pomdp)
...
POMDPs.states(pomdp::DronePOMDP) = pomdp

Base.length(pomdp::DronePOMDP) = pomdp.map_size[1]*pomdp.map_size[2]*2^length(pomdp.rocks_positions) + 1

# we define an iterator over it
function Base.iterate(pomdp::DronePOMDP, i::Int=1)
    if i > length(pomdp)
        return nothing
    end
