...
const SAMPLE_ACTION = (0,0)  # Sampling action

"""
    generate_actions(pomdp::DronePOMDP)

Generate a list of all possible actions: flying to any position, sampling, or sensing a rock.
"""
function generate_actions(pomdp::DronePOMDP{K}) where K
    nx, ny = pomdp.map_size
    all_actions = Vector{Tuple{Int,Int}}()
    ...
    # Add the sampling action (0,0)
    push!(all_actions, SAMPLE_ACTION)

    # Add sensing actions for each rock (0, k)
    for k in 1:K
        push!(all_actions, (0, k))
    end
...
Return the full action space, including flying to any location, sampling, and sensing.
"""
POMDPs.actions(pomdp::DronePOMDP) = generate_actions(pomdp)
...
function POMDPs.actions(pomdp::DronePOMDP, s::RSState)
    if isterminal(pomdp, s)
        return [SAMPLE_ACTION]  # Only sampling allowed in the terminal state
    end
    return generate_actions(pomdp)
end